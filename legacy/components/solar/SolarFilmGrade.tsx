...
/* =============================================================
   Look "film" condiviso delle scene cinematografiche di GM Solar
   (hero + case study). Tre layer puramente decorativi, sovrapposti
   alla scena video, per dare grading, profondità e materia:

   - grade    → velatura calda nell'accent del mondo (chartreuse)
   ...
   quindi restano coerenti anche con prefers-reduced-motion. Le scene che
   vogliono farli "respirare" allo scroll animano via GSAP le classi
   `.solar-grade` / `.solar-vignette` (vedi SolarHero).

   I layer espongono classi stabili così la scena padre può pilotarli con
   un selettore scopato al proprio contesto GSAP, senza ref incrociati.
   ============================================================= */
...
export type SolarFilmGradeProps = {
  className?: string;
 ...
 */
export default function SolarFilmGrade({
  className,
  grade = true,
  gradeOpacity = 0.35,
  vignette = true,
  ...
  return (
    <div aria-hidden className={cn("pointer-events-none absolute inset-0 overflow-hidden", className)}>
      {grade && (
        <div
          className="solar-grade absolute inset-0"
          style={{
      ...
      {vignette && (
        <div
          className="solar-vignette absolute inset-0"
          style={{
      ...
      <div
        className="solar-grain absolute inset-0"
        style={{
