        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {checkout === "done" ? (
            ...
            <div className="text-muted flex h-full flex-col items-center justify-center text-center">
              ...
              <Button href="/shop#catalogo" variant="ghost" className="mt-4" onClick={handleClose}>
                Vai al catalogo
              </Button>
            </div>
          ) : (
...
function QtyButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      aria-label={label}
      className="border-border not-disabled:hover:border-accent flex h-7 w-7 items-center justify-center rounded-full border text-base leading-none transition-colors disabled:cursor-not-allowed disabled:opacity-40"
    >
      {children}
    </button>
  );
