...
// Limiti difensivi: l'endpoint è pubblico, quindi non lo lasciamo aperto.
const MAX_BODY_BYTES = 32 * 1024; // ~32 KB di payload JSON
const MAX_MESSAGES = 20; // turni di conversazione tenuti
const MAX_CONTENT_CHARS = 2000; // caratteri per messaggio

/** Tiene solo i campi attesi, scarta input malformati e LIMITA dimensioni. */
function sanitizeMessages(input: unknown): ChatMessage[] {
  if (!Array.isArray(input)) return [];
  const out: ChatMessage[] = [];
  // Itera al massimo gli ultimi MAX_MESSAGES*2 elementi: niente lavoro illimitato
  // su array enormi inviati ad arte.
  for (const item of input.slice(-MAX_MESSAGES * 2)) {
    if (!item || typeof item !== "object") continue;
    const m = item as { role?: unknown; content?: unknown };
    const role = m.role === "assistant" ? "assistant" : m.role === "user" ? "user" : null;
    if (!role || typeof m.content !== "string" || !m.content.trim()) continue;
    out.push({ role, content: m.content.slice(0, MAX_CONTENT_CHARS) });
  }
...
export async function POST(req: Request) {
  ...
  // Rifiuta payload sproporzionati prima ancora di leggerli.
  const declaredLength = Number(req.headers.get("content-length") ?? 0);
  if (declaredLength > MAX_BODY_BYTES) {
    return Response.json({ type: "error", message: "Richiesta troppo grande." }, { status: 413 });
  }
